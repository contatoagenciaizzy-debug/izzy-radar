...
// Status → display label (português)
...
export class AgentSprite {
  ...
  constructor(
    ...
    // Name badge — above avatar head (avatar center y-70, head top ≈ y-91, badge at y-158)
    // badge height = 62px (3 lines: name, status, detail) → bottom at y-96, 5px gap above avatar top
    const labelY = y - 158;

    // Background pill behind name + status + detail
    this.badgeBg = scene.add.graphics();

    // Name text — bold, clean, high contrast (role prefix + first name, e.g. "Estrategista Diana")
    ...
    // Status text — colored with outline (português: ocioso/trabalhando/concluído/checkpoint/entregando)
    ...
    // Detail text — what the agent is doing right now (e.g. "Escrevendo post 3 de 8")
    ...
    // Click-to-select: clicking the avatar or desk opens the detail panel in React
    this.avatar.setInteractive({ useHandCursor: true });
    this.deskTable.setInteractive({ useHandCursor: true });
    const emitClick = () => this.scene.events.emit('agentClicked', this.agent.id);
    this.avatar.on('pointerdown', emitClick);
    this.deskTable.on('pointerdown', emitClick);
    this.avatar.on('pointerover', () => this.avatar.setScale(AVATAR_SCALE * 1.08));
    this.avatar.on('pointerout', () => this.setAvatarFrame(this.avatar.texture.key));
  }
  ...
  /** Small celebratory "✓" burst shown once when an agent finishes a task — pure visual polish, no state change. */
  private playCompletionBurst(): void {
    const burst = this.scene.add.text(this.avatar.x, this.avatar.y - 40, '✓', {
